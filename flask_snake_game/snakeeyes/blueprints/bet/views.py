...
def place_bet():
	if request.method=='GET':
		recent_bet = Bet.query.filter(Bet.user_id == current_user.id)\
					order_by(Bet.created_on.desc()).limit(10)
		return render_template('bet/place_bet.html', recent_bets= recent_bet)
	...
	if form.validate_on_submit():
		...
		return render_json(200, bet.to_json())
	else:
